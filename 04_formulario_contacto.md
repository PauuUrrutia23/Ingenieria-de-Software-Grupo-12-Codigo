...
`ArchivoAdjunto::create(['archivo_pdf' => $contenidoBinario, ...])` persiste el string binario de PHP directamente. Eloquent con el driver `pgsql` convierte automáticamente strings PHP a `BYTEA` de PostgreSQL. No se necesita codificación adicional.
...
`Visitante::firstOrCreate(['email' => $email], [...])` busca primero por email. Si el visitante cambia su nombre entre consultas, el nombre almacenado será el del **primer registro**. Si se requiere actualizar nombre/apellido en visitas posteriores, reemplazar por:
