...
> **DBRouterController NO se enruta.** `App\Http\Controllers\DBRouterController` (`02b_dbrouter_controller.md`) es el intermediario de base de datos: lo inyecta el contenedor en `AuthController`, `ContactoController`, `ProyectoController`, `InstitucionalCtrl`, `AdminController`, el middleware `AdminAuth` y el job `EnviarEmailBloqueoJob`. No tiene rutas ni aparece en `routes/web.php`. Tras crear la clase, ejecutar `composer dump-autoload` para que el autoloader la registre.

---

## 1b. ORDEN DE CONSTRUCCIÓN / LECTURA DE LOS ARCHIVOS

```
01  → Setup Laravel + migraciones PostgreSQL
02  → Modelos Eloquent (9 modelos)
02b → DBRouterController  ← intermediario BD; requerido por todos los controladores
03  → Autenticación (AuthController, AdminAuth, Job)  — usa DBRouter
04  → Formulario de contacto (ContactoController)     — usa DBRouter
05  → Navegación pública (sin acceso a BD)
06  → Galería de proyectos (ProyectoController)       — usa DBRouter
07  → Certificaciones (ProyectoController, InstitucionalCtrl) — usa DBRouter
08  → Admin proyectos (AdminController)               — usa DBRouter
09  → Admin colaboradores (AdminController)           — usa DBRouter
10  → Integración y pruebas (este archivo)
```

> **Importante para el generador:** crear `DBRouterController` (02b) **antes** de generar cualquier controlador de dominio. Todos lo reciben por inyección de constructor (o por `handle()` en el job) y no contienen llamadas directas a Eloquent.
...
### 3.0 Verificación arquitectónica — DBRouterController

Antes de las pruebas funcionales, confirmar que la capa de acceso a datos respeta el diseño (C_DBRouter como intermediario único):

- [ ] Existe `app/Http/Controllers/DBRouterController.php` con los métodos de `02b_dbrouter_controller.md`.
- [ ] `grep -rEn "::(where|create|find|firstOrCreate|select|whereIn)\(|->save\(\)" app/Http/Controllers app/Http/Middleware app/Jobs --include="*.php"` **no** devuelve coincidencias **fuera** de `DBRouterController.php` (toda la persistencia está centralizada).
- [ ] `AuthController`, `ContactoController`, `ProyectoController`, `InstitucionalCtrl` y `AdminController` declaran `public function __construct(private readonly DBRouterController $db) {}`.
- [ ] `AdminAuth` (middleware) inyecta `DBRouterController` por constructor.
- [ ] `EnviarEmailBloqueoJob::handle(DBRouterController $db)` recibe el router por método (no por constructor).
- [ ] `php artisan route:list` **no** muestra ninguna ruta apuntando a `DBRouterController`.
- [ ] La aplicación arranca sin errores de resolución del contenedor (`php artisan optimize:clear` y una request a `/`).
