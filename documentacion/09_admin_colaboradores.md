# 09 — Módulo de Colaboradores (Panel Admin)
## Plataforma Web Ingecon — Especificación Técnica

> **Destinatario:** Modelo de lenguaje generador de código.  
> **Propósito:** Especificación completa del módulo de colaboradores del panel administrativo: rutas protegidas, métodos del controlador, gestión de BYTEA de logotipos y vista Blade con Alpine.js.  
> **Requerimiento cubierto:** RF46 / CU 7.3.  
> **Dependencias:**
> - Middleware `admin.auth` definido en `03_autenticacion.md`
> - Modelo `Colaborador` según `02_modelos_eloquent.md`
> - Layout `layouts.admin` definido en `08_admin_proyectos.md`
> - `AdminController` existente según `08_admin_proyectos.md` — agregar métodos, no reemplazar
> - **Acceso a datos:** `AdminController` ya recibe `DBRouterController` por constructor (`08_admin_proyectos.md` / `02b_dbrouter_controller.md`). Estos métodos delegan en `$this->db`; no hay llamadas directas a Eloquent.

---

## Consideración de Esquema: Campo `tipo_mime` en `COLABORADOR`

La tabla `COLABORADOR` actual no incluye un campo `tipo_mime`. Para soportar logotipos en múltiples formatos (JPG, PNG, SVG, WebP) con el Data URI correcto, se requiere **agregar la columna `tipo_mime`** a la tabla y al modelo.

### Migración adicional requerida

**Archivo:** `database/migrations/2024_01_01_000010_add_tipo_mime_to_colaborador_table.php`
...
### Actualizar `$fillable` del modelo `Colaborador`

Agregar `'tipo_mime'` al array `$fillable` en `app/Models/Colaborador.php`:
...
└── Http/
    └── Controllers/
        └── AdminController.php           ← agregar indexColaboradores() y storeColaborador()

resources/views/
└── admin/
    └── colaboradores.blade.php           ← nueva vista
```
...
Agregar dentro del grupo `admin.auth` existente, junto a las rutas de proyectos:
...
## 2. Métodos en `AdminController`

Agregar los siguientes métodos a la clase existente en `app/Http/Controllers/AdminController.php`.  
**No reemplazar** los métodos `indexProyectos()`, `storeProyecto()` ni `updateProyecto()` ya definidos en `08_admin_proyectos.md`.

```php
<?php

// Agregar estos use al bloque existente si no están presentes:
...
## 3. Vista Blade — `admin/colaboradores.blade.php`

**Archivo:** `resources/views/admin/colaboradores.blade.php`
...
function adminColaboradores() {
    return {
        ...
        archivoLogotipo: null,     // referencia al objeto File seleccionado
        errorLogotipo:  '',
        ...
        previewLogotipo(event) {
            ...
            // Generar Data URL local para la vista previa
        ...
        async submitAgregar() {
            ...
            // Construir FormData — Content-Type se establece automáticamente
            ...
            try {
                const res = await fetch('/admin/colaboradores', {
                    ...
                    headers: {
                        'Accept':       'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        // NO incluir Content-Type — el navegador gestiona el boundary
                    },
                    body: fd,
                });
                ...
                if (data.success) {
                    // Agregar al inicio de la lista sin recargar
...
```
[Admin] click "Agregar Colaborador"
    → modalAgregar = true → modal visible

[Admin] escribe nombre_comercial
    → x-model actualiza form.nombre_comercial

[Admin] selecciona archivo de logotipo
    → previewLogotipo(event):
        ├─ archivo.type.startsWith('image/') ? ✓ : error
        ├─ archivo.size <= 2MB ? ✓ : error
        ├─ FileReader.readAsDataURL()
        └─ logotipoPreview = data URL → <img> visible

[Admin] click "Guardar"
    → submitAgregar():
        ├─ Validación frontend (nombre vacío, logotipo faltante)
        ├─ FormData: nombre_comercial + logotipo (File object)
        ├─ fetch POST /admin/colaboradores (multipart/form-data)
        └─ AdminController@storeColaborador:
               ├─ Validación Laravel (mimes, max)
               ├─ file_get_contents() → BYTEA
               ├─ getMimeType() → tipo_mime almacenado
               ├─ $this->db->crearColaborador(...)  → Colaborador::create(...)
               └─ JSON {success: true, colaborador: {..., logotipo_base64}}
        → colaboradores.unshift(data.colaborador)  ← reactivo sin recarga
        → cerrarModal()
```
...
### FileReader vs Data URI del servidor

El preview usa `FileReader.readAsDataURL()` para generar un Data URL **localmente** en el navegador antes del envío. Esto evita una petición previa al servidor solo para mostrar el preview y es instantáneo. El Data URI final que se guarda en la lista de colaboradores proviene del servidor (`data.colaborador.logotipo_base64`), garantizando que refleja exactamente lo almacenado en BYTEA.

### SVG y tipo MIME

Los archivos SVG tienen MIME `image/svg+xml`. El Data URI generado resultante sería `data:image/svg+xml;base64,...`. Los navegadores modernos renderizan SVG correctamente en `<img>` con este formato. El campo `tipo_mime` en la BD almacena el valor exacto retornado por `$archivo->getMimeType()`, que para SVG es `image/svg+xml`.

### Validación de tipo en frontend vs backend

El frontend usa `archivo.type.startsWith('image/')` como primera línea de defensa. El backend usa la regla `mimes:jpg,jpeg,png,svg,webp` de Laravel, que verifica los **magic bytes** del archivo (no solo la extensión). Ambas capas son complementarias y necesarias.

### `object-contain` para logotipos

Los logotipos corporativos suelen tener proporciones variadas (horizontales, cuadrados, etc.). `object-contain` garantiza que el logotipo se muestre completo dentro del contenedor `w-24 h-24` sin recorte ni distorsión, con el fondo `bg-slate-50` visible en los espacios vacíos.

### Migración adicional es obligatoria

Sin el campo `tipo_mime` en la tabla `colaborador`, todos los logotipos no-PNG se servirán con el Data URI incorrecto `data:image/png;base64,...`, lo que puede causar que SVGs y WebPs no rendericen correctamente en algunos navegadores. Ejecutar la migración `000010` antes de usar el módulo.
