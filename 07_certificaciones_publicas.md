...
### Método privado DRY (opcional)

Si se prefiere evitar duplicación entre `certificaciones()` e `index()`, extraer la query a un método privado:

```php
private function obtenerCertificadosActivos(): \Illuminate\Database\Eloquent\Collection
{
    $certs = Certificado::select([
            'id_certificado', 'codigo_lote', 'fecha_emision', 'estado', 'id_proyecto',
        ])
        ->where('estado', 'activo')
        ->with(['proyecto' => fn($q) => $q->select(['id_proyecto', 'nombre_obra', 'region'])])
        ->orderBy('fecha_emision', 'desc')
    ...
    return $certs->transform(function (Certificado $cert) {
        $cert->fecha_formateada = $cert->fecha_emision?->format('d/m/Y') ?? '—';
        return $cert;
    });
}
