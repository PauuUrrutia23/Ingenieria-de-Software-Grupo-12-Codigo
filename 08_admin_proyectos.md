...
class AdminController extends Controller
{
    ...
    public function indexProyectos(Request $request): JsonResponse
    {
        ...
        $proyectos = Proyecto::where('id_admin', $admin->id_admin)
            ...
            ->withCount('imagenes')   // genera columna imagenes_count
    ...
    public function storeProyecto(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'proyecto' => [
                ...
                'cantidad_imagenes'  => $proyecto->imagenes()->count(),
                'thumbnail_base64'   => $thumbnail,
            ],
        ], 201);
    }
    ...
    public function updateProyecto(Request $request, int $id): JsonResponse
    {
        ...
        $cantActual   = $proyecto->imagenes()->count();
        ...
        // ------------------------------------------------------------------
        $primeraImagen = $proyecto->imagenes()->orderBy('id_imagen')->first();
        $thumbnail     = null;
...
`Proyecto::withCount('imagenes')` genera la columna `imagenes_count` mediante un subquery SQL eficiente. No requiere cargar las imágenes completas en memoria solo para contar.

### Prevención de eliminación cruzada

En `updateProyecto()`, la eliminación de imágenes siempre incluye `.where('id_proyecto', $proyecto->id_proyecto)`:
